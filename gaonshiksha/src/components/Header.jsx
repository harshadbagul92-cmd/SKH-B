        ...
        <div className="flex items-center justify-between h-16">
          ...
          <div className="flex items-center space-x-3 lg:hidden">
            ...
            <div className="flex items-center space-x-2">
              ...
              <span className="font-black text-base tracking-tight text-[#0097A7]">
                INVICTUS
              </span>
            </div>
          </div>
          ...
          <div className="hidden lg:flex items-center space-x-3">
            <div className="flex items-center space-x-2 text-xs font-bold text-slate-500">
              <span className="text-[#0097A7]">Invictus Learning</span>
              <span>/</span>
              <span className="text-[#0097A7] capitalize">
          ...
          <div className="flex items-center space-x-3 sm:space-x-4">
            ...
            {pendingSyncCount > 0 && (
              <button
                onClick={triggerSync}
                className="flex items-center space-x-1 text-xs font-bold bg-[#FFEB01]/20 text-[#0097A7] border border-[#FFEB01] px-3 py-1.5 rounded-xl hover:bg-[#FFEB01]/30 transition-colors"
                title="Sync offline changes"
              >
                <RefreshCw className="w-3.5 h-3.5 animate-spin text-[#0097A7]" />
                  ...
                  {languages.map((l) => (
                    <button
                      ...
                      className={`w-full text-left px-3 py-2 text-xs font-semibold flex items-center justify-between hover:bg-blue-50 transition-colors ${
                        lang === l.code ? 'text-[#0097A7] font-bold bg-blue-50/50' : 'text-slate-700'
                      }`}
                    >
                      <span>{l.label}</span>
                      {lang === l.code && <span className="text-[#0097A7]">✓</span>}
                    </button>
                  ))}
                </div>
              )}
            ...
            >
              <div className="w-6 h-6 rounded-lg bg-[#0097A7] text-[#FFEB01] font-black text-xs flex items-center justify-center">
                {userProfile?.name?.charAt(0) || 'S'}
              </div>
              <span className="text-xs font-bold text-[#0097A7] hidden sm:inline max-w-[120px] truncate">
